...
export const TOOLS: Record<string, Tool> = {
  ...
  // ── TikTok content posting (Sloane / Astra) ──
  // The storefront's tiktok-post edge function owns auth + media selection;
  // these tools just drive it. Posts default to SELF_ONLY (private) until
  // the TikTok app passes review — pass privacy explicitly to change that,
  // and only with the owner's standing instruction (guardrail #4).
  tiktok_post_photo: {
    spec: {
      name: "tiktok_post_photo",
      description: "Post a product photo carousel to the Luveni TikTok account (content-posting API, not TikTok Shop). Picks the best ≤9 images from product_media. Private (SELF_ONLY) by default until app review passes.",
      parameters: {
        type: "object",
        properties: {
          productId: { type: "string" },
          title: { type: "string", description: "Post title (≤90 chars)" },
          description: { type: "string", description: "Caption/hashtags" },
          privacy: { type: "string", enum: ["SELF_ONLY", "PUBLIC_TO_EVERYONE", "MUTUAL_FOLLOW_FRIENDS", "FOLLOWER_OF_CREATOR"] },
        },
        required: ["productId"],
      },
    },
    run: async (i) => {
      const res = await fetch(`${process.env.SUPABASE_URL}/functions/v1/tiktok-post`, {
        method: "POST",
        headers: { "content-type": "application/json", Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}` },
        body: JSON.stringify({ action: "post_photo", product_id: i.productId, title: i.title, description: i.description, privacy: i.privacy }),
      });
      return await res.json().catch(() => ({ ok: res.ok, status: res.status }));
    },
  },
  ...
  tiktok_post_status: {
    spec: {
      ...
      description: "Check the publish status of a TikTok post by publish_id (from tiktok_post_photo).",
      parameters: { type: "object", properties: { publishId: { type: "string" } }, required: ["publishId"] },
    },
    run: async (i) => {
      const res = await fetch(`${process.env.SUPABASE_URL}/functions/v1/tiktok-post`, {
        method: "POST",
        headers: { "content-type": "application/json", Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}` },
        body: JSON.stringify({ action: "status", publish_id: i.publishId }),
      });
      return await res.json().catch(() => ({ ok: res.ok, status: res.status }));
    },
  },
